...
def mp_worker(worker_id, req_q, resp_q, pool_resp_q, episode_q, cfg):
    ...
    def new_game():
        ...
        cap = (min(max_plies, resume + curr_mcts_tail) if curriculum
               else max_plies)          # bound the MCTS tail (curriculum only)
        ...
        def _new_game(self):
            ...
            if self.curriculum:
                ...
                if ok and not st.is_terminal():
                    state, actions = st, list(pref)
            cap = (min(self.max_plies, resume + self.curr_mcts_tail)
                   if self.curriculum else self.max_plies)
            slot = {'state': state, 'hist': [], 'aux': [], 'actions': actions,
                    'move': resume, 'resume': resume, 'sims': sims, 'cap': cap,
                    'root': None, 'n': 0, 'pool': None}
